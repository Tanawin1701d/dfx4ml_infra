...
\addr{0x0040} & \reg{MAIN\_STATE} & 4 bit & \badro &
  Main flow state machine output:\newline
  \textbf{0x0}~=~SHUTDOWN\newline
  \textbf{0x1}~=~PROCESS\newline
  \textbf{0x2}~=~PRE\_SHUTDOWN \\

\addr{0x0080} & \reg{RECON\_STATE} & 4 bit & \badro &
...
\addr{0x00C0} & \reg{EXEC\_STATE} & 4 bit & \badro &
...
\addr{0x0100} & \reg{LAST\_SESSION} & 3 bit & \badrw &
...
\addr{0x0240} & \reg{PR\_IP\_ADDR} & 32 bit & \badrw &
...
\addr{+0x14} & \reg{PROF\_EXEC} & 32 bit & \badrw &
...
\addr{+0x18} & \reg{VS\_RM\_RECON} \newline \reg{\_SELECT} & 2 bit & \badrw &
...
\addr{+0x20} & \reg{LOAD\_MASK} & 8 bit & \badrw &
...
\addr{+0x24} & \reg{STORE\_MASK} & 8 bit & \badrw &
...
\addr{+0x28} & \reg{COMPLETE\_MASK} & 8 bit & \badrw &
...
\addr{+0x2C} & \reg{NEXT\_SESSION} & 3 bit & \badrw &
...
\begin{enumerate}
  \item \textbf{Main state machine} (\reg{MAIN\_STATE}) --- the top-level sequencer with three states:
    \begin{enumerate}[label=\alph*.]
      \item \textbf{SHUTDOWN}: the DFX Manager is idle. All Bank~0 and Bank~1 registers are writable from the PS (except read-only status registers). The machine waits for the PS to issue \textbf{CTRL\_START} via \reg{CTRL\_REG}.
      \item \textbf{PROCESS}: both the RECON and EXEC sub-machines are launched in parallel. The machine stays in PROCESS until both sub-machines reach FIN\_SYNC simultaneously.
      \item \textbf{PRE\_SHUTDOWN}: a single-cycle transition that advances the session pointer to \reg{NEXT\_SESSION}, increments the DMA source/destination addresses for the next iteration, and decides whether to loop back to PROCESS (query budget not yet exhausted) or transition to SHUTDOWN. If \reg{INTR\_ENA} is set and all queries are done, \reg{INTR\_STATUS} is asserted.
    \end{enumerate}

  \item \textbf{Reconfiguration sub-machine} (\reg{RECON\_STATE}) --- controls the DFX CTRL IP in parallel with EXEC:
    \begin{enumerate}[label=\alph*.]
      \item \textbf{REPROG}: drives \texttt{dfx\_rm\_program} with \reg{VS\_RM\_RECON\_SELECT} to trigger partial-region bitstream loading.
      \item \textbf{W4SLAVERESET}: waits for the partial-region reset signal (\texttt{dfx\_rm\_nreset}) to deassert.
      \item \textbf{W4SLAVEOP}: waits for \texttt{dfx\_rm\_nreset} to reassert (reconfiguration complete).
      \item \textbf{FIN\_SYNC}: holds until the EXEC sub-machine also reaches FIN\_SYNC, then both return to SHUTDOWN together.
    \end{enumerate}

  \item \textbf{Execution sub-machine} (\reg{EXEC\_STATE}) --- initializes and triggers the data-path IPs in parallel with RECON:
    \begin{enumerate}[label=\alph*.]
      \item \textbf{INITIALIZE\_PR\_CTRL}: sends \texttt{batch\_size} and \texttt{ap\_start} commands to the PR-controlled IP (address from \reg{PR\_IP\_ADDR}) via the PR CTRL Connector.
      \item \textbf{CLEAR\_MGS}: one-cycle pulse to reset all DFX Streamer channels specified by \reg{LOAD\_MASK}.
      \item \textbf{INITIALIZE\_MGS}: one-cycle pulse to initialize the DFX Streamer channels specified by \reg{STORE\_MASK}.
      \item \textbf{INITIALIZE\_DMA}: resets the DMA interrupt via the DMA Connector.
      \item \textbf{SET\_DMA\_LOAD}: programs the DMA engine with the source address and size from the current session slot (\reg{DMA\_SRC\_ADDR} / \reg{DMA\_SRC\_SIZE}). Skipped if \reg{LOAD\_MASK}[0] is clear.
      \item \textbf{SET\_DMA\_STORE}: programs the DMA engine with the destination address and size (\reg{DMA\_DES\_ADDR} / \reg{DMA\_DES\_SIZE}). Skipped if \reg{STORE\_MASK}[0] is clear.
      \item \textbf{TRIGGERING}: one-cycle pulse to start the DMA transfer.
      \item \textbf{WAIT4FIN}: waits until \reg{COMPLETE\_MASK}~$=$~\reg{STORE\_MASK} (all expected store operations have completed).
      \item \textbf{FIN\_SYNC}: holds until the RECON sub-machine also reaches FIN\_SYNC, then both return to SHUTDOWN together.
    \end{enumerate}
\end{enumerate}
