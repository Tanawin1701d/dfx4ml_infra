...
% ── Main state machine ───────────────────────────────────────────────────────
\textbf{1.~Main state machine} (\reg{MAIN\_STATE}) --- top-level sequencer:
\begin{enumerate}[label=\alph*., leftmargin=2em]\raggedright
  \item \textbf{SHUTDOWN}: idle; Bank~0/1 registers are PS-writable.
        Waits for \reg{CTRL\_REG}~$=$~\texttt{CTRL\_START}.
  \item \textbf{PROCESS}: launches RECON and EXEC in parallel;
        holds until both reach \textbf{FIN\_SYNC} simultaneously.
  \item \textbf{PRE\_SHUTDOWN}: single-cycle housekeeping --- advances the
        session pointer (\reg{NEXT\_SESSION}), increments DMA addresses, then
        either loops back to \textbf{PROCESS} or returns to \textbf{SHUTDOWN}.
        Asserts \reg{INTR\_STATUS} if \reg{INTR\_ENA} is set and all queries
        are complete.
\end{enumerate}

\medskip
% ── Reconfiguration sub-machine ──────────────────────────────────────────────
\textbf{2.~Reconfiguration sub-machine} (\reg{RECON\_STATE}) ---
controls the DFX CTRL IP in parallel with EXEC:
\begin{enumerate}[label=\alph*., leftmargin=2em]\raggedright
  \item \textbf{REPROG}: reads \reg{VS\_RM\_RECON\_SELECT} from Bank~1.\\
        \hspace*{1em}$\bullet$~If~$0$: skip to \textbf{FIN\_SYNC}
          (bypasses b and c).\\
        \hspace*{1em}$\bullet$~Otherwise: drives \texttt{dfx\_rm\_program}
          $\to$ \textbf{W4SLAVERESET}.
  \item \textbf{W4SLAVERESET}: waits for \texttt{dfx\_rm\_nreset} to deassert.
  \item \textbf{W4SLAVEOP}: waits for \texttt{dfx\_rm\_nreset} to reassert
        (reconfiguration complete).
  \item \textbf{FIN\_SYNC}: holds until EXEC also reaches \textbf{FIN\_SYNC};
        both return to \textbf{SHUTDOWN} together.
\end{enumerate} 

\medskip
% ── Execution sub-machine ────────────────────────────────────────────────────
\textbf{3.~Execution sub-machine} (\reg{EXEC\_STATE}) ---
initializes and triggers the data-path IPs in parallel with RECON:
\begin{enumerate}[label=\alph*., leftmargin=2em]\raggedright
  \item \textbf{PR\_CTRL\_REQ\_SKIP\_CHECK}: entry state;
        reads \reg{VS\_RM\_EXEC\_SELECT} from Bank~1.\\
        \hspace*{1em}$\bullet$~If~$0$: skip to \textbf{CLEAR\_MGS}.\\
        \hspace*{1em}$\bullet$~Otherwise: arm PR CTRL sequencer
          $\to$ \textbf{INITIALIZE\_PR\_CTRL}.
  \item \textbf{INITIALIZE\_PR\_CTRL}: sends \texttt{batch\_size} and
        \texttt{ap\_start} to the PR-controlled IP (\reg{PR\_IP\_ADDR});
        advances on \texttt{AP\_START} completion.
  \item \textbf{CLEAR\_MGS}: one-cycle reset of DFX Streamer channels
        in \reg{LOAD\_MASK}.
  \item \textbf{INITIALIZE\_MGS}: one-cycle init of DFX Streamer channels
        in \reg{STORE\_MASK}.
  \item \textbf{INITIALIZE\_DMA}: resets the DMA interrupt flag.
  \item \textbf{SET\_DMA\_LOAD}: programs DMA source address and size
        (\reg{DMA\_SRC\_ADDR}, \reg{DMA\_SRC\_SIZE});
        skipped if \reg{LOAD\_MASK}[0]~$= 0$.
  \item \textbf{SET\_DMA\_STORE}: programs DMA destination address and size
        (\reg{DMA\_DES\_ADDR}, \reg{DMA\_DES\_SIZE});
        skipped if \reg{STORE\_MASK}[0]~$= 0$.
  \item \textbf{TRIGGERING}: one-cycle pulse to start the DMA transfer.
  \item \textbf{WAIT4FIN}: waits until
        \reg{COMPLETE\_MASK}~$=$~\reg{STORE\_MASK}.
  \item \textbf{FIN\_SYNC}: holds until RECON also reaches \textbf{FIN\_SYNC};
        both return to \textbf{SHUTDOWN} together.
\end{enumerate} 
