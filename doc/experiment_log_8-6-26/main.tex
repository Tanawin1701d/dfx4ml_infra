...
\title{DFX4ML-ARCH (Progress Update 8-6-26)}
\author{
...
In the four-part scheme, the model is split at the boundaries of the four sequential parts and mapped onto two physical reconfigurable regions over four reconfiguration phases, as shown in Figure~\ref{fig:model_4sub}. The part 1 and 3 are loaded into \emph{region~0}, and the part 2 and 4 into \emph{region~1}; the two regions are reconfigured alternately, so that one region can execute its current part while the other is being reconfigured for the next. 
...
The four parts are mapped onto two PR regions (\texttt{pblock\_dfx\_pr\_region\_0/1}), each region hosting two RM variants. Figure~\ref{fig:res_skip4_util} reports the utilisation of each part against the device, with reference lines at the full device limit and at the 50\% per-region (static region included)
...
In parallel, we plan to upgrade the DFX Manager bank~0 register set to track the
performance of the latency-hiding technique directly in hardware. By exposing
on-chip counters that measure the overlap between reconfiguration and
data/compute phases, we can cross-validate the latency-hiding gains.
