#!/usr/bin/env python3
"""Visualise the four-part (skip4) DFX pipeline as a two-lane timeline.

Reads the ``skip4_exp_perf`` table from the project DuckDB result store and
...
so the timeline reflects the double-buffered schedule, where region R is
reconfigured while the previously-loaded region is still executing.
...
TABLE    = "skip4_exp_perf"
OUT_DIR  = os.path.dirname(os.path.abspath(__file__))
OUT_PNG  = os.path.join(OUT_DIR, "skip4_perf.png")

CLK_FREQ = 99999001.0          # build clock (Hz), ~100 MHz -> cycles -> ms
NUM_RM   = 2                   # RM variants per PR region (skip4: 2 regions x 2 RMs)
...
# ----------------------------------------------------------------------------
def load_df():
    """Return the skip4_exp_perf table as a DataFrame, ordered by slot.
...
def main():
    ...
    ax.set_title("Four-part (skip4) DFX pipeline timeline "
