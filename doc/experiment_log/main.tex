...
\subsubsection{Multi-Region (IN-PROGRESS)}

Before modifying the whole system to support the multi DFX region, we would like to do a small experiment to learn how to use it for multi-region. The thing that we consider include:

\begin{enumerate}
  \item DFX Ctrl IP:
  \begin{enumerate}
    \item Explore how to manage the signal for multi-region
    \item Explore increasing the ICAP3 clock to 200 MHz (the maximum clock rate recommended for ICAP3 on Zync Ultrascale Plus) to increase the transfer rate from $\sim$400 MB/s to $\sim$800 MB/s while maintaining the rest of fabric at 100 MHz (ML + Static region)
  \end{enumerate}

  \item DFX Manager IP (Magic Sequencer):
  \begin{enumerate}
    \item We consider to upgrade bank 1 to support the latency hiding technique but We don't have the certain architecture right now.
    \item The DFX Manager is right now huge and hard to maintain in Verilog, We might consider to replace it with SystemVerilog. (I think Claude can do this perfectly)
  \end{enumerate}

  \item DFX Manager IP (Magic Sequencer):
  \begin{enumerate}
    \item We need to upgrade the tcl to enable the multi-region connection and DFX Streamer arrangement.
  \end{enumerate}
